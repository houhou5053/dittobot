...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!args[1]) {
            ...
            request(`https://mcsrvstat.us/server/${args[0].toString()}`, function(err, body) {
                if (!err) {
                    try {
                        var status = body.split("<h2>")[1].split("&")[0].toString();

                        if (status === "Could not get the server status") {
                            embed.setDescription(`**${args[0]}** 서버는 현재 **오프라인**입니다.`)
                            message.channel.send(embed)
                        }
                    } catch {
                        embed.setDescription(`**${args[0]}** 서버는 현재 **온라인**입니다.`)
                        message.channel.send(embed)
                    }
                }
            });
